...
const TribuneForm: React.FC<TribuneFormProperties> = ( { matchs } ) => {
  ...
  const matchOption = matchs.map( ( match ) => { return { value: match.id, label: `${match.phase}, ${match.date}, ${match.time}` } } )
  ...
  return (
    <form className='bg-rose-500' onSubmit={handleSubmit( onSubmit )}>
      ...
      <ImageUpload onChange={( value ) => setImage( value )}>test</ImageUpload>
      <div>
